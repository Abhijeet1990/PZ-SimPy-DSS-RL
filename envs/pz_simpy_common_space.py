...
def reinitialize_midsize_network():
        ...
        routers[1].out_channel.append(channels[4])
        routers[1].out_channel.append(channels[5])
        routers[2].out_channel.append(channels[6])
        routers[2].out_channel.append(channels[7])
        routers[3].out_channel.append(channels[8])
        routers[4].out_channel.append(channels[9])
        routers[5].out_channel.append(channels[10])
        routers[6].out_channel.append(channels[11])
        routers[7].out_channel.append(channels[12])
...
class CyberEnv(BaseEnv):
    ...
    def observation_space(self, agent):
        ...
        return Box(low=0, high=100.0, shape=(4,), dtype=np.float32)
    ...
    def step(self, action):
        ...
        if self._agent_selector.is_last():
            ...
            for ix,agent in enumerate(self.agents):
                ...
                if agent == 'R1':
                    self.observations[agent] = obs_all[:4]
                elif agent == 'R2':
                    self.observations[agent] = obs_all[4:8]
                elif agent == 'R3':
                    self.observations[agent] = obs_all[8:]
                else:
