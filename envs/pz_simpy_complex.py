    ...
    def getReward(self):
        try:
            ...
            for rtr in self.routers:
                rewards.append((1.0-float(rtr.packets_drop)/float(rtr.packets_rec))*reward)
            return rewards
        except:
...
def reinitialize_midsize_network():
        ...
        channels : List[Channel] = [
            ...
            Channel(SimMan.env, 'C34',src = routers[2],dest=routers[3]),
            ...
            Channel(SimMan.env, 'C56',src = routers[4],dest=routers[5]),
            Channel(SimMan.env, 'C57',src = routers[4],dest=routers[6]),
            Channel(SimMan.env, 'C67',src = routers[5],dest=routers[6])
            #,
        ...
        routers[2].out.append(routers[3])
        routers[2].out.append(routers[4])
        routers[2].out.append(routers[5])
        routers[3].out.append(routers[4])
        routers[3].out.append(routers[5])
        routers[4].out.append(routers[5])
        routers[4].out.append(routers[6])
        routers[5].out.append(routers[6])
        routers[6].out.append(interpreter)

        channels.append(Channel(SimMan.env, 'CS',src = routers[6],dest=interpreter))

        # initialize the channels
        senders[0].out_channel = channels[0]
        senders[1].out_channel = channels[1]
        routers[0].out_channel.append(channels[2])
        routers[1].out_channel.append(channels[3])
        routers[1].out_channel.append(channels[4])
        routers[2].out_channel.append(channels[5])
        routers[2].out_channel.append(channels[6])
        routers[2].out_channel.append(channels[7])
        routers[3].out_channel.append(channels[8])
        routers[3].out_channel.append(channels[9])
        routers[4].out_channel.append(channels[10])
        routers[4].out_channel.append(channels[11])
        routers[5].out_channel.append(channels[12])
        routers[6].out_channel.append(channels[13])
    ...
    def step(self, action):
        ...
        if self._agent_selector.is_last():
            ...
            for ix,agent in enumerate(self.agents):
                ...
                if agent in ['R1','R6','R7'] and action > 0:
                    self.rewards[agent] = -1
                elif agent in ['R2','R4','R5'] and action > 1:
