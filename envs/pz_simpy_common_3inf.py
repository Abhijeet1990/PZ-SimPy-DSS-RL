...
from envs.utils_simpy_common import BaseEnv,Interpreter,Message,StackMessageTypes
...
class ReceiverDevice(Interpreter):
    ...
    def getReward(self):
        try:
            ...
            if self.id == "PS":
                reward = 0
                for k, v in self.packets_record.items():
                    # linear rewarding
                    if v > 0:
                        reward+=0.1*v
                    else:
            ...
            # get the rewards for the first 3 routers
            for rtr in self.routers[:3]:
                score = 0
                for r in rtr.out:
                    score += -float(r.packets_drop)/float(r.packets_rec)
                rewards.append(score*reward)
            return rewards
        except:
            return [0.0 for i in range(len(self.routers))]

    def getRewardBackUp(self):
        try:
            rewards = []
            if self.id == "PS":
            ...
            for rtr in self.routers:
                if reward >0:
                    rewards.append((1.1-float(rtr.packets_drop)/float(rtr.packets_rec))*reward)
                else:
                    rewards.append(reward/(1.1-float(rtr.packets_drop)/float(rtr.packets_rec)))
            return rewards
        except:
            return [0.0 for i in range(len(self.routers))]
    ...
    def getObservation(self):
        observations = []
        for router in self.routers[:3]:
            for ch in router.out_channel:
                observations.append(ch.utilization_rate)
            for r in router.out:
                observations.append(r.packets_drop)
    ...
    def getDone(self):
        if self.id == "PS":
            ...
            # bounding it to maximum 20 steps
            if len(gthan5) == 2 or self.env.now >= 1000.0:
                self._done = True
...
def reinitialize_midsize_network():
        ...
        routers: List[ForwarderDevice] = [
            ...
            ForwarderDevice(SimMan.env,'R3',rate=200.0, qlimit=200),
            ForwarderDevice(SimMan.env,'R4',rate=200.0, qlimit=200),
            ForwarderDevice(SimMan.env,'R5',rate=200.0, qlimit=200),
            ForwarderDevice(SimMan.env,'R6',rate=200.0, qlimit=200),
            ForwarderDevice(SimMan.env,'R7',rate=200.0, qlimit=200),
            ForwarderDevice(SimMan.env,'R8',rate=200.0, qlimit=200)
        ]
        routers[to_target].qlimit=0.0
        ...
        channels : List[Channel] = [
            ...
            Channel(SimMan.env, 'C16',src = routers[0],dest=routers[5]),
            Channel(SimMan.env, 'C23',src = routers[1],dest=routers[2]),
            Channel(SimMan.env, 'C25',src = routers[1],dest=routers[4]),
            Channel(SimMan.env, 'C27',src = routers[1],dest=routers[6]),
            Channel(SimMan.env, 'C36',src = routers[2],dest=routers[5]),
            Channel(SimMan.env, 'C37',src = routers[2],dest=routers[6]),
            Channel(SimMan.env, 'C38',src = routers[2],dest=routers[7]),
            Channel(SimMan.env, 'C46',src = routers[3],dest=routers[5]),
            Channel(SimMan.env, 'C57',src = routers[4],dest=routers[6]),
            Channel(SimMan.env, 'C68',src = routers[5],dest=routers[7]),
            Channel(SimMan.env, 'C78',src = routers[6],dest=routers[7])
            #,
        ...
        routers[0].out.append(routers[5])
        routers[1].out.append(routers[2])
        routers[1].out.append(routers[4])
        routers[1].out.append(routers[6])
        routers[2].out.append(routers[5])
        routers[2].out.append(routers[6])
        routers[2].out.append(routers[7])
        routers[3].out.append(routers[5])
        routers[4].out.append(routers[6])
        routers[5].out.append(routers[7])
        routers[6].out.append(routers[7])
        routers[7].out.append(interpreter)

        channels.append(Channel(SimMan.env, 'CS',src = routers[7],dest=interpreter))
        ...
        routers[1].out_channel.append(channels[7])
        routers[2].out_channel.append(channels[8])
        routers[2].out_channel.append(channels[9])
        routers[2].out_channel.append(channels[10])
        routers[3].out_channel.append(channels[11])
        routers[4].out_channel.append(channels[12])
        routers[5].out_channel.append(channels[13])
        routers[6].out_channel.append(channels[14])
        routers[7].out_channel.append(channels[15])
...
class CyberEnv(BaseEnv):
    ...
    def observation_space(self, agent):
        ...
        return Box(low=0, high=100.0, shape=(6,), dtype=np.float32)
    ...
    def step(self, action):
        ...
        if self._agent_selector.is_last():
            ...
            for ix,agent in enumerate(self.agents):
                obs_all, rewards, self.terminations[agent], self.infos[agent] = _interpreter.getFeedback()

                # obtain the channel utilization and router packet drop rate
                if agent == 'R1':
                    self.observations[agent] = obs_all[:6]
                elif agent == 'R2':
                    self.observations[agent] = obs_all[6:12]
                elif agent == 'R3':
                    self.observations[agent] = obs_all[12:]
                else:
                    print('Not a correct agent')

                self.rewards[agent] = rewards[ix]
