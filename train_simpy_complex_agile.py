...
try:
    ...
    INIT_HP["MAX_ACTION"] = [cenv.action_space(agent).high for agent in cenv.agents]
    INIT_HP["MIN_ACTION"] = [cenv.action_space(agent).low for agent in cenv.agents]

# Append number of agents and agent IDs to the initial hyperparameter dictionary
INIT_HP["N_AGENTS"] = cenv.num_agents
INIT_HP["AGENT_IDS"] = cenv.agents
...
max_steps = 20  # Maximum steps to take in each episode
...
for idx_epi in trange(max_episodes):
    for agent in pop:  # Loop through population
        state, info = cenv.reset()  # Reset environment at start of episode
        agent_reward = {agent_id: 0 for agent_id in cenv.agents}
        ...
        for _ in range(max_steps):
            ...
            next_state, reward, termination, truncation, info = cenv.step(
                action
            )  # Act in environment
...
path = "./models/MATD3"
filename = "simpy_MATD3_trained_agent_v2.pt"
